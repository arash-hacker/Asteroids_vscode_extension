...
export function activate(context: vscode.ExtensionContext) {
	let disposable = vscode.commands.registerCommand('extension.asteroid', async () => {
       ...
       let sourceCode=await fetch("https://www.managroup.co/index.html").then(r=>{
        let s=r.text() ; 
       console.log(s);
         return s;
         
        });
        sourceCode=sourceCode.replace(/\.\/static/g,"https://www.managroup.co/static")
...
function getWebSite(jsPath:vscode.Uri,sourceCode:string,which:string) {
  ...
  console.log(sourceCode.substr(0,1000));
